...
void tls_close_free(void_t handle) {
    ...
    uv_tls_close((uv_tls_t *)h, (uv_tls_close_cb)ZE_FREE);
}
...
void on_connect_handshake(uv_tls_t *tls, int status) {
    ZE_ASSERT(tls->tcp_hdl->data == tls);
