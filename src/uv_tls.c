...
void evt_tls_set_role(evt_tls_t *t, evt_endpt_t role) {
    ZE_ASSERT(t != NULL && (role == ENDPT_IS_CLIENT || role == ENDPT_IS_SERVER));
...
static int evt__send_pending(evt_tls_t *conn) {
    ...
    ZE_ASSERT(buf != NULL && "Memory alloc failed");
    if (!buf) return 0;

    int p = BIO_read(conn->app_bio, buf, pending);
    ZE_ASSERT(p == pending);

    ZE_ASSERT(conn->writer != NULL && "You need to set network writer first");
...
static int evt__tls__op(evt_tls_t *conn, enum tls_op_type op, void *buf, int sz) {
    ...
    switch (op) {
        ...
        case EVT_TLS_OP_WRITE:
            {
                ZE_ASSERT(sz > 0 && "number of bytes to write should be positive");
        ...
        default:
            ZE_ASSERT(0 && "Unsupported operation");
            break;
    }
    return r;
...
int evt_tls_feed_data(evt_tls_t *c, void *data, int sz) {
    ...
    int rv = 0;
    int i = 0;
    ZE_ASSERT(data != NULL && "invalid argument passed");
    ZE_ASSERT(sz > 0 && "Size of data should be positive");
...
int evt_tls_accept(evt_tls_t *tls, evt_handshake_cb cb) {
    ...
    //ZE_ASSERT( tls->reader != NULL && "You need to set network reader first");
...
static void alloc_cb(uv_handle_t *handle, size_t size, uv_buf_t *buf)
{
    ...
    ZE_ASSERT(buf->base != NULL && "Memory allocation failed");
}
...
static void on_hd_complete( evt_tls_t *t, int status)
{
    ...
    ZE_ASSERT( ut != NULL && ut->tls_hsk_cb != NULL);
...
{
    uv_tls_t *utls = (uv_tls_t*)handle->data;
    ZE_ASSERT( utls->tls_cls_cb != NULL);
    evt_tls_free(utls->tls);
