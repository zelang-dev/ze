...
void co_delete(routine_t *handle) {
    ...
    } else if (!(handle->status == ZE_NORMAL || handle->status == ZE_DEAD || handle->status == ZE_EVENT_DEAD) && !handle->exiting) {
...
ZE_FORCE_INLINE void co_resuming(routine_t *handle) {
