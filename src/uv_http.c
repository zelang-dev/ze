...
bool has_variable(http_t *this, string key, string var) {
    char temp[ZE_SCRAPE_SIZE] = {0};

    snprintf(temp, ZE_SCRAPE_SIZE, "%s%s", var, "=");
    return is_str_in(get_header(this, key), temp);
}

bool has_flag(http_t *this, string key, string flag) {
    char flag1[ZE_SCRAPE_SIZE] = {0};
    char flag2[ZE_SCRAPE_SIZE] = {0};
    char flag3[ZE_SCRAPE_SIZE] = {0};
    string value = get_header(this, key);

    snprintf(flag1, ZE_SCRAPE_SIZE, "%s%s", flag, ";");
    snprintf(flag2, ZE_SCRAPE_SIZE, "%s%s", flag, ",");
    snprintf(flag3, ZE_SCRAPE_SIZE, "%s%s", flag, "\r\n");
