...
ZE_FORCE_INLINE string co_strdup(string_t str) {
    return co_memdup(co_active(), str, strlen(str));
}

ZE_FORCE_INLINE string co_string(string_t str, size_t length) {
...
int vasprintf(string *str_p, string_t fmt, va_list ap) {
    ...
    *str_p = (string)ZE_MALLOC(requiredSize);
...
string co_str_concat(string_t header, string_t *words, size_t num_words) {
    ...
    for (int i = 0; i < num_words; ++i) {
        ...
        message = (string)ZE_REALLOC(message, message_len);
        strncat(strncat(message, ";", message_len), words[i], message_len);
    }

    co_defer(ZE_FREE, message);
    return message;
}
