...
future *future_create(callable_t start_routine) {
    future *f = ZE_CALLOC(1, sizeof(future));
...
void async_start(future *f, promise *value, void_t arg) {
    future_arg *f_arg = ZE_CALLOC(1, sizeof(future_arg));
    f_arg->func = f->func;
    f_arg->arg = arg;
    f_arg->value = value;
    int r = pthread_create(&f->thread, &f->attr, future_wrapper, f_arg);
    ZE_INFO("thread #%lx created thread #%lx with status(%d) future id(%d) \n", co_async_self(), thread_id(f->thread), r, f->id);
}
...
promise *promise_create() {
    promise *p = ZE_CALLOC(1, sizeof(promise));
    p->result = ZE_CALLOC(1, sizeof(values_t));
    ...
    p->id = rand();
    p->done = false;
    ZE_INFO("promise id(%d) created in thread #%lx\n", p->id, co_async_self());

    return p;
}

void promise_set(promise *p, void_t res) {
    ZE_INFO("promise id(%d) set LOCK in thread #%lx\n", p->id, co_async_self());
    pthread_mutex_lock(&p->mutex);
    p->result->value.object = res;
    p->done = true;
    pthread_cond_signal(&p->cond);
    ZE_INFO("promise id(%d) set UNLOCK in thread #%lx\n", p->id, co_async_self());
    pthread_mutex_unlock(&p->mutex);
}

value_t promise_get(promise *p) {
    ZE_INFO("promise id(%d) get LOCK in thread #%lx\n", p->id, co_async_self());
    pthread_mutex_lock(&p->mutex);
    while (!p->done) {
        ZE_INFO("promise id(%d) get WAIT in thread #%lx\n", p->id, co_async_self());
        pthread_cond_wait(&p->cond, &p->mutex);
    }
    ZE_INFO("promise id(%d) get UNLOCK in thread #%lx\n", p->id, co_async_self());
    pthread_mutex_unlock(&p->mutex);
