...
static void co_deferred_internal(routine_t *coro, defer_func_t *deferred) {
    ...
    ZE_ASSERT(num_defers != 0 && deferred != NULL);
...
ZE_FORCE_INLINE size_t co_defer(func_t func, void_t data) {
...
ZE_FORCE_INLINE void co_defer_cancel(size_t index) {
    co_deferred_cancel(co_active(), index);
}

ZE_FORCE_INLINE void co_defer_fire(size_t index) {
